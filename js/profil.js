...
/**
 * Calcule le métabolisme de base (formule de Mifflin-St Jeor)
 */
...
async function sauvegarderProfil(donnees) {
  ...
  if (donnees.id) {
    ...
    for (const p of tousLesProfils) {
      p.actif = 0;
      await dbPut(NEXXAT_DB.stores.profils, p);
    }
...
async function afficherProfil() {
  ...
  const mb         = calculerMetabolismeBase(profilActif.poids, profilActif.taille, profilActif.age, profilActif.sexe);
...
function remplirFormulaireProfil() {
  if (!profilActif) return;
  const champs = ['nom', 'prenom', 'age', 'sexe', 'taille', 'poids', 'objectifPoids', 'niveauActivite'];
...
async function handleNouveauPoids(e) {
  ...
  afficherProfil();
  document.getElementById('form-nouveau-poids').reset();
  chargerGraphiquePoids();
}

/**
 * Charge et affiche le graphique d'évolution du poids (canvas simple)
 */
async function chargerGraphiquePoids() {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    const valeur = max - (max - min) * i / 4;
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.font = '10px Orbitron, sans-serif';
    ctx.fillText(valeur.toFixed(1), 2, y + 4);
  }
  ...
  if (profilActif.objectifPoids) {
    ...
    ctx.font = '10px Orbitron, sans-serif';
    ctx.fillText(`Objectif: ${profilActif.objectifPoids}kg`, w - 100, yObj - 5);
  }
}
