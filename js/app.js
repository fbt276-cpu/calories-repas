...
// ── Résumé calories sur l'accueil ────────────────────────
async function mettreAJourResumeAccueil() {
  if (!profilActif) return;
  const repasJour = await chargerRepasJour();
  const totaux    = calculerTotaux(repasJour);
  const besoins   = calculerBesoinsCaloriques(
  ...
  const pct     = Math.min(Math.round((totaux.calories / besoins) * 100), 100);

  const elCal = document.getElementById('nutri-calories-accueil');
  const elRest = document.getElementById('nutri-restant-accueil');
  const barre  = document.getElementById('barre-calories-accueil');
  if (elCal)  elCal.textContent  = `${Math.round(totaux.calories)} kcal`;
  if (elRest) elRest.textContent = `${Math.round(restant)} restantes`;
  if (barre)  { barre.style.width = `${pct}%`; barre.style.background = pct > 100 ? '#e74c3c' : '#ff8c00'; }
}
